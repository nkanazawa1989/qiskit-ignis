...
def create_fake_cr_rabi_experiment_result(
        target_qubits: Tuple[int, int],
        amps: np.ndarray,
        fit_args: np.ndarray,
        iq_phase: float,
        data_slots: Tuple[int, int],
        n_qubits: int,
        exp_id: str) -> Result:
    """Create fake experimental result of rough CR amplitude calibration.
    Only target qubit (second qubit index) shows oscillation with respect to the CR amplitude.
    ...
    Args:
        target_qubits: Index of control and target qubit.
        amps: Array of pulse amplitudes.
        fit_args: Fit parameters. This conforms to the fit function of
            :py:class:~`qiskit.ignis.experiments.calibration.analysis.CosinusoidalFit`.
        iq_phase: IQ phase of the measure centroid on the IQ plane.
        data_slots: Index of data slot where measurement data is stored.
        n_qubits: Number of qubit in this system.
        exp_id: Strings representing the experimental ID. This is usually generated
            by the Ignis experiment class.
    """
    ...
    for amp in amps:
        data = np.zeros(n_qubits, dtype=complex)
        iq_amp = fit_args[0] * np.cos(2 * np.pi * fit_args[1] * amp + fit_args[2]) + fit_args[3]
        data[data_slots[1]] = iq_amp * np.exp(1j * iq_phase)
        result_dict = {
            'shots': 1024,
            'success': True,
            'data': {'memory': list(map(list, np.vstack((data.real, data.imag)).T))},
    ...
    for amp in amps:
        meta_dict = {
            'name': 'rough_amplitude',
            'register_map': dict(zip(target_qubits, data_slots)),
            'series': 'single_pulse',
            'x_values': {'cr90p.u{:d}.gaussian_square.amp'.format(target_qubits[1]): amp},
            'pulse_schedule_name': 'SinglePulseGenerator',
            'exp_id': exp_id,
